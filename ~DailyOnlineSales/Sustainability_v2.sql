...
WHERE sih.BusinessDate >= @StartDate
AND sih.IsReturn = 'N'
AND sih.Status = 'A'
UNION ALL
SELECT 
...
SELECT 
	...
	spi.ItemCode [ItemCodeSips],
	s.RegisterPrice,
	DATEDIFF(DAY, ISNULL(ss.firstScannedOn, spi.DateInStock), s.BusinessDate) [DaysToSell_Retail]
INTO #RetailSales
FROM #temp_RetailSales s
	INNER JOIN ReportsData.dbo.SipsProductInventory spi
		ON	LEFT(s.ItemCode, 1) <> '0'							--Used items start non-zero values in the sales tables
...
SELECT
	...
	tbd.SipsItemCode,
	DATEDIFF(DAY, ISNULL(ss.firstScannedOn, spi.DateInStock), tbh.UpdateTime) [DaysToDisposal]
INTO #Disposed
...
SELECT 
	...
	om.AmazonSalesRank,
	DATEDIFF(DAY, ISNULL(ss.firstScannedOn, spi.DateInStock), ISNULL(om.RefundDate, om.ShipDate)) [DaysToSell_iStore]
INTO #iStoreSales
...
WHERE 
	 ...
	 AND od.MarketSKU LIKE 'S_%'

SELECT 
*
FROM #ScanSummary sa
	LEFT OUTER JOIN #Disposed d
		ON sa.ItemCode = d.SipsItemCode
	LEFT OUTER JOIN #RetailSales rs
		ON sa.ItemCode = rs.ItemCodeSips
	LEFT OUTER JOIN #iStoreSales iss
		ON sa.ItemCode = iss.ItemCode
ORDER BY sa.firstScannedOn

SELECT 
	DATEADD(DAY, DATEDIFF(DAY, 0, ss.firstScannedOn), 0) [BusinessDate],
	COUNT(ss.ItemCode) [countShelvedItem],
	COUNT(CASE WHEN ss.isCurrent = 1
		THEN ss.ItemCode
	...
	AVG(rs.DaysToSell_Retail) [avg_RetailDaysToSell],
		COUNT(CASE WHEN rs.DaysToSell_Retail >= 0   AND rs.DaysToSell_Retail < 7  THEN ss.ItemCode END)		[count_RetailSoldItems_under7days],
		COUNT(CASE WHEN rs.DaysToSell_Retail >= 7   AND rs.DaysToSell_Retail < 28 THEN ss.ItemCode END)		[count_RetailSoldItems_7to28days],
		COUNT(CASE WHEN rs.DaysToSell_Retail >= 28  AND rs.DaysToSell_Retail < 56 THEN ss.ItemCode END)		[count_RetailSoldItems_28to56days],
		COUNT(CASE WHEN rs.DaysToSell_Retail >= 56  AND rs.DaysToSell_Retail < 112 THEN ss.ItemCode END)	[count_RetailSoldItems_56to112days],
		COUNT(CASE WHEN rs.DaysToSell_Retail >= 112 THEN ss.ItemCode END)									[count_RetailSoldItems_over112days],
	COUNT(iss.iStoreOrder) [count_iStoreSoldItems],
	AVG(iss.DaysToSell_iStore) [avg_iStoreDaysToSell],
		COUNT(CASE WHEN iss.DaysToSell_iStore >= 0   AND iss.DaysToSell_iStore < 7  THEN ss.ItemCode END)	[count_iStoreSoldItems_under7days],
		COUNT(CASE WHEN iss.DaysToSell_iStore >= 7   AND iss.DaysToSell_iStore < 28 THEN ss.ItemCode END)	[count_iStoreSoldItems_7to28days],
		COUNT(CASE WHEN iss.DaysToSell_iStore >= 28  AND iss.DaysToSell_iStore < 56 THEN ss.ItemCode END)	[count_iStoreSoldItems_28to56days],
		COUNT(CASE WHEN iss.DaysToSell_iStore >= 56  AND iss.DaysToSell_iStore < 112 THEN ss.ItemCode END)	[count_iStoreSoldItems_56to112days],
		COUNT(CASE WHEN iss.DaysToSell_iStore >= 112 THEN ss.ItemCode END)									[count_iStoreSoldItems_over112days]
FROM #Calendar c
	LEFT OUTER JOIN #ScanSummary ss
		ON c.BusinessDate = DATEADD(DAY, DATEDIFF(DAY, 0, ss.firstScannedOn), 0)
	LEFT OUTER JOIN #Disposed d
...
GROUP BY DATEADD(DAY, DATEDIFF(DAY, 0, ss.firstScannedOn), 0)
