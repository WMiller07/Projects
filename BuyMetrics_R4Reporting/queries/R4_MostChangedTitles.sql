...
SELECT 
	...
	CAST(COUNT(CASE
				...
				THEN bbi.CatalogID
				END) AS FLOAT) / CAST(NULLIF(COUNT(bbi.CatalogID), 0) AS FLOAT) [pct_ChainSO],
	CAST(COUNT(CASE
				WHEN bbi.SuggestedOfferType = 2
				THEN bbi.CatalogID
				END) AS FLOAT) / CAST(NULLIF(COUNT(bbi.CatalogID), 0) AS FLOAT) [pct_LocationSO],
	CAST(COUNT(CASE
				WHEN (bbi.Offer / bbi.Quantity) <> bbi.SuggestedOffer
				AND bbi.SuggestedOfferType = 1
				THEN bbi.CatalogID
				END) AS FLOAT) / CAST(NULLIF(COUNT(bbi.CatalogID), 0) AS FLOAT) [pct_ChangedChainSO],
	CAST(COUNT(CASE
				WHEN (bbi.Offer / bbi.Quantity) <> bbi.SuggestedOffer
				AND bbi.SuggestedOfferType = 2
				THEN bbi.CatalogID
				END) AS FLOAT) / CAST(NULLIF(COUNT(bbi.CatalogID), 0) AS FLOAT) [pct_ChangedLocationSO],
	CAST(SUM(CASE
				WHEN bbi.SuggestedOfferType = 1
	...
	CAST(SUM(CASE
				...
				CAST(SUM(CASE
							WHEN bbi.SuggestedOfferType = 2
							THEN bbi.Quantity
							END) AS FLOAT) [avg_LocationSOAmt],
	CAST(SUM(CASE
				WHEN (bbi.Offer / bbi.Quantity) <> bbi.SuggestedOffer
				THEN bbi.Offer
				END) AS FLOAT) / 
...
GROUP BY t.CatalogID, t.isbn13, t.title, t.author
HAVING 
