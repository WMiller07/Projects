...
SELECT 
	...
	p.rollavg_CountBuyTransactions/NULLIF(LAG(p.rollavg_CountBuyTransactions, 12) OVER (ORDER BY p.BusinessMonth), 0) - 1 [change_countBuyTransactions],
	p.rollavg_CountBuyItems/NULLIF(LAG(p.rollavg_CountBuyItems, 12) OVER (ORDER BY p.BusinessMonth), 0) - 1 [change_countBuyItems]
...
SELECT 
	ppc.LocationNo,
	ppc.BusinessMonth,
	ppc.rollavg_Income / ppi.rollavg_Income - 1 [rollavg_Income],
	ppc.rollavg_Payroll / ppi.rollavg_Payroll -1 [rollavg_Payroll],
	ppc.rollavg_PayrollPct - ppi.rollavg_PayrollPct [rollavg_PayrollPct],
	ppc.change_Income - ppi.change_Income [change_Income],
	ppc.change_Payroll - ppi.change_Payroll [change_Payroll],
	ppc.change_PayrollPct - ppi.change_PayrollPct [change_PayrollPct],
	LAG(ppc.change_Payroll, 3) OVER (PARTITION BY ppc.LocationNo ORDER BY ppc.BusinessMonth) - 
		LAG(ppi.change_Payroll, 3) OVER (PARTITION BY ppi.LocationNo ORDER BY ppi.BusinessMonth) [change3mo_Payroll],
	LAG(ppc.change_PayrollPct, 3) OVER (PARTITION BY ppc.LocationNo ORDER BY ppc.BusinessMonth) -
		LAG(ppi.change_PayrollPct, 3) OVER (PARTITION BY ppi.LocationNo ORDER BY ppi.BusinessMonth) [change3mo_PayrollPct],
	LAG(ppc.change_Payroll, 6) OVER (PARTITION BY ppc.LocationNo ORDER BY ppc.BusinessMonth) - 
		LAG(ppi.change_Payroll, 6) OVER (PARTITION BY ppi.LocationNo ORDER BY ppi.BusinessMonth) [change6mo_Payroll],
	LAG(ppc.change_PayrollPct, 6) OVER (PARTITION BY ppc.LocationNo ORDER BY ppc.BusinessMonth) - 
		LAG(ppi.change_PayrollPct, 6) OVER (PARTITION BY ppi.LocationNo ORDER BY ppi.BusinessMonth) [change6mo_PayrollPct],

	LAG(ppc.change_countBuyTransactions, 3) OVER (PARTITION BY ppc.LocationNo ORDER BY ppc.BusinessMonth) - 
		LAG(ppi.change_countBuyTransactions, 3) OVER (PARTITION BY ppi.LocationNo ORDER BY ppi.BusinessMonth) [change3mo_BuyTransactions],
	LAG(ppc.change_countBuyItems, 3) OVER (PARTITION BY ppc.LocationNo ORDER BY ppc.BusinessMonth) -
		LAG(ppi.change_countBuyItems, 3) OVER (PARTITION BY ppi.LocationNo ORDER BY ppi.BusinessMonth) [change3mo_BuyItems],
	LAG(ppc.change_countBuyTransactions, 6) OVER (PARTITION BY ppc.LocationNo ORDER BY ppc.BusinessMonth) - 
		LAG(ppi.change_countBuyTransactions, 6) OVER (PARTITION BY ppi.LocationNo ORDER BY ppi.BusinessMonth) [change6mo_BuyTransactions],
	LAG(ppc.change_countBuyItems, 6) OVER (PARTITION BY ppc.LocationNo ORDER BY ppc.BusinessMonth) - 
		LAG(ppi.change_countBuyItems, 6) OVER (PARTITION BY ppi.LocationNo ORDER BY ppi.BusinessMonth) [change6mo_BuyItems]
FROM #PayrollPctChanges ppc
	INNER JOIN #PayrollPctIndexChange ppi
		ON ppc.BusinessMonth = ppi.BusinessMonth
