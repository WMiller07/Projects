...
WHERE slm.OpenDate <= DATEADD(YEAR, -2, @StartDate)
AND slm.ClosedDate IS NULL
AND slm.StoreType = 'S'
AND slm.LocationNo NOT IN ('00001', '00005')
...
FROM rHPB_Historical..BuyHeaderHistory bhh
	INNER JOIN MathLab..StoreLocationMaster slm
		...
		AND slm.ClosedDate IS NULL
WHERE bhh.Status = 'A'
	AND bhh.BusinessDate >= @StartDate
...
SELECT 
	p.LocationNo,
	p.BusinessMonth,
	p.rollavg_Income [rollavg_Income],
	p.rollavg_Payroll [rollavg_Payroll],
	p.rollavg_PayrollPct [rollavg_PayrollPct],
	p.rollavg_Income/NULLIF(LAG(p.rollavg_Income, 12) OVER (PARTITION BY p.LocationNo ORDER BY p.BusinessMonth), 0) [change_Income],
	p.rollavg_Payroll/NULLIF(LAG(p.rollavg_Payroll, 12) OVER (PARTITION BY p.LocationNo ORDER BY p.BusinessMonth), 0) [change_Payroll],
	(p.rollavg_PayrollPct ) - 
		(LAG(p.rollavg_PayrollPct, 12) OVER (PARTITION BY p.LocationNo ORDER BY p.BusinessMonth)) [change_PayrollPct],
	p.rollavg_CountBuyTransactions/NULLIF(LAG(p.rollavg_CountBuyTransactions, 12) OVER (PARTITION BY p.LocationNo ORDER BY p.BusinessMonth), 0) [change_countBuyTransactions],
	p.rollavg_CountBuyItems/NULLIF(LAG(p.rollavg_CountBuyItems, 12) OVER (PARTITION BY p.LocationNo ORDER BY p.BusinessMonth), 0) [change_countBuyItems]
...
SELECT 
	...
	LAG(ppc.change_Income, 3) OVER (PARTITION BY ppc.LocationNo ORDER BY ppc.BusinessMonth) [change3mo_Income],
	LAG(ppc.change_Income, 6) OVER (PARTITION BY ppc.LocationNo ORDER BY ppc.BusinessMonth) [change6mo_Income],
	LAG(ppc.change_Income, 12) OVER (PARTITION BY ppc.LocationNo ORDER BY ppc.BusinessMonth) [change12mo_Income]
FROM #PayrollPctChanges ppc
	INNER JOIN #Locations loc
		ON ppc.LocationNo = loc.LocationNo
